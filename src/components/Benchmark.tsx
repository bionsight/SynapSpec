...
/* ── 추세 차트 ────────────────────────────────────────────────────────────
   ...
   ⚠ 이 컴포넌트는 목록 화면에서 hydration 불일치(React #418)를 만든다.
   원인을 아직 못 찾았다. 확인한 것과 아닌 것:

   - 이 컴포넌트를 빼면 목록 화면 콘솔이 깨끗해진다. 넣으면 다시 난다.
     A/B 로 확인했으므로 원인은 이 안에 있다.
   - 상세 화면과 /about, /download, / 는 깨끗하다. 차트가 없는 화면들이다.
   - 달 이름을 toLocaleString 에서 배열로 바꾼 것(5f9b5a1)은 이 오류를 고치지
     못했다. 그 커밋의 메시지는 고쳤다고 적었으나 사실이 아니다 — 그때 콘솔을
     페이지가 hydration 되기 전에 읽었다. 로케일 의존을 없앤 것 자체는 맞다.
   - SVG <text> 안의 자식 텍스트 노드를 한 덩어리 문자열로 합쳐도 그대로다.

   화면은 정상이다. React 가 서버 HTML 을 버리고 클라이언트에서 다시 그린다.
   목록 화면이 unlisted 인 동안은 급하지 않지만, 공개 전에는 잡아야 한다. */
